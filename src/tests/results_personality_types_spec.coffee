...
QUnit.asyncTest("Results Personality Types Widget Appears with load('perosnalityTypes'...)", (assert)->
...
QUnit.asyncTest("Results Personality Types Widget Appears with load('perosnalityTypes'...)", (assert)->
  ...
  builder.onInitialize(->
    waitUntil(->
      ...
      index = document.querySelectorAll(".widget .personality-type")[3].getAttribute("data-index")
      document.querySelectorAll(".widget .personality-type")[3].trigger('click')
